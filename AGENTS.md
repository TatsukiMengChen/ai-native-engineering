...
11. **Fumadocs 默认组件保护规范**
    - **禁止盲目覆盖 `defaultMdxComponents` 中的组件**：Fumadocs 的默认 MDX 组件（如 `Table`、`Image` 等）自带 Tailwind 类名和布局结构（如 `overflow-auto`、`prose-no-margin`、`my-6`）。在覆盖前必须先查阅 Fumadocs 源码确认默认行为。
    - **样式 Bug 优先排查 CSS 层叠**：当遇到渲染异常时，先检查项目自定义 CSS（如 `page.module.css`、`fuwari.css`）是否通过更高优先级的选择器覆盖了 Fumadocs/Tailwind 的默认样式，而非盲目在组件层添加包裹容器。
    - **覆盖组件时必须保留原有 className**：如果确实需要自定义组件，必须保留 Fumadocs 原有组件的所有 Tailwind 类名（可通过查看 `fumadocs-ui/src/mdx.tsx` 确认）。

12. **PR 与提交卫生规范**
    - **禁止提交临时文件**：`test-results/`、`playwright-report/`、`.last-run.json`、`package-lock.json`（当项目使用 bun 时）等临时产物不得进入 Git 历史。**新增测试框架时必须同步更新 `.gitignore`**。
    - **计划文档不入仓库**：`implementation-plan.md` 等一次性计划文档的内容应写入 PR description，而非作为文件提交到代码库（除非本文件明确要求保留）。
    - **PR 前必须做 Code Review**：在向上游发起 PR 之前，必须以 Senior Reviewer 视角自查变更，重点关注：
      1. 是否破坏了 Fumadocs 主题样式或默认组件行为
      2. 是否存在 CSS 层叠覆盖导致的意外副作用
      3. SSR/Hydration 安全性（虽然本项目使用静态导出，但仍需确认组件渲染的确定性）
      4. 是否包含了不必要的文件或临时产物
